...
public class Enqservice {
    ...
    @Transactional
    public void deleteServiceByBookId(Long bookId) {
        if (!enqrepo.existsById(bookId)) {
            throw new EntityNotFoundException("Service not found for event ID: " + bookId);
        }
        enqrepo.deleteById(bookId);
    }

    public List<Enqentity> getfood(long s) {
        return enqrepo.getchefid(s);
    }
    public Enqdto updateStatusById(long bookingID, String status) {
        Enqentity existingService = enqrepo.findById(bookingID);
        if (existingService != null) {
